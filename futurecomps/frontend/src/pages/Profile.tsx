...
export default function Profile() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // stay on settings page or show success
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${user.isVerified ? 'bg-green-500' : 'bg-yellow-500'}`} />
                        <p className="text-2xl font-bold text-foreground">{user.isVerified ? 'Verified' : 'Unverified'}</p>
                    </div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                 ...
                 {/* Removed generic Member Since if createdAt is not available yet, or handle it safely */}
                 <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                    <h3 className="text-sm font-medium text-muted-foreground mb-1">Member Since</h3>
                    {/* @ts-ignore - createdAt might be missing in type but present in data */}
                    <p className="text-2xl font-bold text-foreground">{user.createdAt ? new Date(user.createdAt).getFullYear() : new Date().getFullYear()}</p>
                </div>
            </div>
          ...
          <div className="max-w-2xl">
            ...
            <form onSubmit={handleUpdate} className="space-y-6">
                ...
                <div className="pt-4 flex items-center gap-4">
                    ...
                    {success && <p className="text-sm text-green-500 font-medium flex items-center gap-1"><CheckCircle2 className="w-4 h-4" /> {success}</p>}
                    {error && <p className="text-sm text-red-500 font-medium flex items-center gap-1"><AlertCircle className="w-4 h-4" /> {error}</p>}
                </div>
            </form>
