...
export default function AdminDashboard() {
  ...
  // Add pendingOrders and lowStockProducts to the type
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      // First try to use the current user from context
      ...
      // For development purposes, you can uncomment this line to bypass auth check
      ...
      try {
        ...
        // This ensures we have the data available for other tabs
        if (dashboardData.recentActivity) {
          if (
            dashboardData.recentActivity.products &&
            dashboardData.recentActivity.products.length > 0
          ) {
            // Fetch full products data to ensure we have complete information
            await fetchProducts();
          }
          ...
          if (
            ...
            // Fetch full orders data to ensure we have complete information
          ...
          if (
            ...
            // Fetch full users data to ensure we have complete information
  ...
  const calculateDashboardStats = (
    ...
    // Set dashboard stats
    // @ts-ignore - pendingOrders and lowStockProducts are used in the UI but not in the type
    setDashboardStats({
      totalRevenue,
    ...
    // In a real app, you would fetch historical data to calculate these
    ...
    // In a real app, you would calculate this based on order items
    // For now, we'll create mock data based on the products
  ...
  useEffect(() => {
    if (activeTab === "orders") {
      ...
      // Always fetch coupons when the tab is selected to get the latest usage data
      fetchCouponsData();
    }
  }, [activeTab]);
  ...
  const handleUpdateOrderStatus = async (
    ...
    // Debug: Log the order ID and status
    console.log("Updating order status:", { orderId, newStatus });

    // Find the order in the local state to get the MongoDB _id
  ...
  */
  ...
  // User management
  // @ts-ignore - These functions are commented out but still referenced in the component
  const handleInspectUser = (user: User) => {
    // These functions are not available anymore
    // @ts-ignore
  ...
  const handleToggleUserStatus = async (
    ...
    try {
      // Use PUT method directly since it's more reliable with CORS
  ...
  const handleChangeUserRole = async (userId: string, newRole: string) => {
    ...
    try {
      // First try with dedicated role endpoint
      try {
        ...
        // If dedicated endpoint fails, try with general update endpoint as fallback
    ...
    <AdminLayout>
      ...
      >
        ...
        <TabsContent value="orders">
          <OrdersTable
            // @ts-ignore - Type mismatch between different Order types
      ...
      <Dialog
        ...
        //@ts-ignore
        onCloseComplete={() => {
          // Force a re-render to ensure the UI is responsive
          setTimeout(() => {
            document.body.style.pointerEvents = "";
          }, 0);
        }}
      ...
      <Dialog
        ...
        //@ts-ignore
        onCloseComplete={() => {
          // This will run after the dialog is fully closed
          if (selectedCoupon) {
            setSelectedCoupon(null);
          }
          // Force a re-render to ensure the UI is responsive
      ...
      {/* Order Details Dialog removed - now using dedicated page */}
    </AdminLayout>
  );
}
