...
import homepageService, { CarouselItem } from "../api/services/homepageService";
import { Link } from "react-router-dom";

// Default collection data in case API fails
...
export default function BoundaryCarousel() {
  ...
  const [collections, setCollections] = useState<CarouselItem[]>([]);
  ...
  useEffect(() => {
    const fetchCarouselItems = async () => {
      try {
        setLoading(true);
        const items = await homepageService.getCarouselItems();

        // If no items returned, use default collections
  ...
  // Get current collection with theme-appropriate values
  const currentCollection =
    collections.length > 0
      ? {
          ...
          backgroundColor:
            theme === "dark"
              ? "#121212" // Darker background for dark mode
              : collections[activeIndex].lightBackground,
          accentColor:
            theme === "dark"
              ? collections[activeIndex].darkAccentColor
              : collections[activeIndex].accentColor,
        }
      : null;

  // Next and previous indices
  const nextIndex =
    collections.length > 0 ? (activeIndex + 1) % collections.length : 0;
  const prevIndex =
    collections.length > 0
      ? (activeIndex - 1 + collections.length) % collections.length
      : 0;

  // Progress bar animation
  const startProgressAnimation = () => {
    if (progressRef.current) {
      // Add a small delay to ensure CSS transitions are applied correctly
      setTimeout(() => {
        if (progressRef.current) {
  ...
  useEffect(() => {
    // Only start autoplay if we have collections and aren't loading
    ...
    const startAutoPlay = () => {
      ...
      // Small delay to ensure animation resets before starting again
      setTimeout(() => {
  ...
  const handleNext = () => {
    ...
    // Use requestAnimationFrame to ensure smooth animation transitions
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          // Wait a bit more before allowing text animations to complete
          setTimeout(() => {
        ...
        <div className="flex-1 flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-2/3 relative overflow-hidden min-h-[400px] sm:min-h-[500px] lg:min-h-[600px]">
            ...
            <div className="absolute inset-0 z-30 pointer-events-none">
              <div className="w-full h-full bg-[url('/diagonal-lines.svg')] bg-repeat opacity-5" />
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/3 flex flex-col justify-center px-4 sm:px-8 lg:px-16 py-8 sm:py-12 relative z-30">
            <AnimatePresence mode="wait">
              ...
              >
                {/* Model number with animated stroke - Responsive sizing */}
                <div
